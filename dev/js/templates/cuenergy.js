...
<div class="container-fluid">
  <div class="row">
      <div class="col-sm-12 event-title-and-location">
          <div><a href="${builtData.event.localist_url}" >${
    builtData.event.title
}</a></div>
          <div><span class="event-date">${builtData.event_date}</span> - ${
...
export const cuenergyEventsWrapper = inner => `
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-sm-2 event-month-and-day">
        <div><span class="event-month">${
            builtData.abbrMonth
        }</span><span class="event-day">${builtData.day}</span></div>
      </div>
      <div class="col-sm-10 event-title-and-location">
        <div><a href="${builtData.event.localist_url}" >${
    builtData.event.title
}</a></div>
        <div>${builtData.event_time}${
    builtData.event.location_name ? ` | ${builtData.event.location_name}` : ''
}</div>
      </div>
    </div>
...
export const cuenergyCompactWrapper = inner => `
